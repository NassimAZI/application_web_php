...
<section id="container">
		<h2>Entrer les personnes ayant effectué leur volontariat</h2>
...
$datte = $_POST['dat'];
$reqw = $bdd->query("SELECT * FROM volontariat WHERE Date LIKE '$datte'");
$rece= $reqw->fetch();
...
$id_ut = intval($donnees['IdUtilisateur']);
// Modifier Aeffectue dans la table ut_cot pour l'utilisateur 
$val=1;
$req2 = $bdd->prepare("UPDATE ut_vol SET A_Participe = :val WHERE IdUtilisateur LIKE :id_ut AND IdVolontariat LIKE :recue");
$req2 -> execute(array('val'=>$val,'id_ut'=>$id_ut,'recue'=>$recue));
